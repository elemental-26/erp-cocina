...
const HR_EVALUATION_TEMPLATE = [
  ...
  { group: "Competencias", items: ["Escucha instrucciones", "Comunicacion", "Trabajo bajo presion", "Trabajo en equipo", "Actitud", "Responsabilidad", "Iniciativa", "Adaptabilidad"] },
...
export default function TalentoHumanoView({
  ...
  usuarios, areas, currentUser, primary, accent,
  onColaboradores, onEvaluaciones, onPlanes, onCapacitaciones, onCertificaciones,
}) {
  ...
  return (
    <div className="space-y-3">
      ...
      {sub === "evaluaciones" && <Evaluaciones colaboradores={colaboradores} evaluaciones={evaluaciones} planes={planes} currentUser={currentUser} primary={primary} onEvaluaciones={onEvaluaciones} onPlanes={onPlanes} />}
...
function Colaboradores({ colaboradores, evaluaciones, certificaciones, areas, usuarios, primary, onColaboradores, onCertificaciones }) {
  const blank = { nombre: "", documento: "", cargo: "", area: areas[0]?.nombre || "", fechaIngreso: dateOnly(new Date()), estado: "Activo", supervisor: "", foto: null };
  const [form, setForm] = useState(blank);
  const [editId, setEditId] = useState(null);
  const [detail, setDetail] = useState(null);

  const save = () => {
    if (!form.nombre.trim() || !form.documento.trim()) return;
    if (editId) {
      onColaboradores(colaboradores.map((c) => c.id === editId ? { ...c, ...form, nombre: form.nombre.trim(), documento: form.documento.trim() } : c));
      setEditId(null);
    } else {
      onColaboradores([{ id: genId(), ...form, nombre: form.nombre.trim(), documento: form.documento.trim() }, ...colaboradores]);
    }
    setForm(blank);
  };
  ...
  return (
    <div className="space-y-3">
      <div className="bg-white rounded-xl p-3 space-y-2">
        <h3 className="font-bold text-sm flex items-center gap-1.5" style={{ fontFamily: "Oswald, sans-serif" }}><UserPlus size={15} /> Base de colaboradores</h3>
        <div className="grid sm:grid-cols-2 gap-2">
          ...
          <select value={form.area} onChange={(e) => setForm({ ...form, area: e.target.value })} className="border rounded-md px-3 py-2 text-sm">
...
function CollaboratorDetail({ colaborador, evaluaciones, certificaciones, onClose, onAddCert, primary }) {
  const [cert, setCert] = useState({ tipo: HR_CERT_TYPES[0], vencimiento: "", alertaDias: 30, notas: "" });
  return (
    <Modal title={colaborador.nombre} onClose={onClose} wide>
      <div className="grid sm:grid-cols-[110px_1fr] gap-3">
        <div className="w-24 h-24 rounded-xl bg-gray-100 overflow-hidden flex items-center justify-center">
          {colaborador.foto ? <img src={colaborador.foto} className="w-full h-full object-cover" alt="" /> : <Users size={28} className="text-gray-300" />}
        </div>
        <div className="text-sm text-gray-600 space-y-1">
          <p><b>ID:</b> {colaborador.id}</p>
          <p><b>Documento:</b> {colaborador.documento}</p>
          <p><b>Cargo:</b> {colaborador.cargo || "Sin cargo"}</p>
          <p><b>Area:</b> {colaborador.area || "Sin area"}</p>
          <p><b>Ingreso:</b> {colaborador.fechaIngreso || "Sin fecha"}</p>
          <p><b>Estado:</b> {colaborador.estado}</p>
      ...
      <div className="space-y-2">
        ...
        <div className="grid sm:grid-cols-4 gap-2">
          <select value={cert.tipo} onChange={(e) => setCert({ ...cert, tipo: e.target.value })} className="border rounded-md px-2 py-1.5 text-sm">
            {HR_CERT_TYPES.map((t) => <option key={t}>{t}</option>)}
          </select>
...
function Evaluaciones({ colaboradores, evaluaciones, planes, currentUser, primary, onEvaluaciones, onPlanes }) {
  const [type, setType] = useState("ingreso");
  const [colaboradorId, setColaboradorId] = useState(colaboradores[0]?.id || "");
  const [periodicidad, setPeriodicidad] = useState(HR_REVIEW_PERIODS[0]);
  const [criteria, setCriteria] = useState(() => criteriaFor("ingreso"));
      ...
      <div className="bg-white rounded-xl p-3 space-y-2">
        <div className="grid sm:grid-cols-3 gap-2">
          ...
          <select value={periodicidad} onChange={(e) => setPeriodicidad(e.target.value)} disabled={type !== "desempeno"} className="border rounded-md px-3 py-2 text-sm disabled:opacity-40">
            {HR_REVIEW_PERIODS.map((p) => <option key={p}>{p}</option>)}
          </select>
        </div>
...
function CriterionRow({ criterion, onChange }) {
  ...
  return (
    <div className="border border-gray-100 rounded-lg p-2">
      ...
      <div className="grid sm:grid-cols-[1fr_130px] gap-2 mt-2">
        ...
        <label className="border rounded-md px-2 py-1.5 text-xs font-bold flex items-center justify-center gap-1 cursor-pointer">
          <ImagePlus size={13} /> {criterion.evidence ? "Evidencia OK" : "Evidencia"}
          <input type="file" accept="image/*" onChange={handleEvidence} className="hidden" />
        </label>
      </div>
    </div>
  );
...
function Indicadores({ colaboradores, evaluaciones, planes, capacitaciones, certificaciones, primary }) {
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(latest.map((c) => ({ Colaborador: c.nombre, Documento: c.documento, Cargo: c.cargo, Area: c.area, Supervisor: c.supervisor, Promedio: c.promedio }))), "Ranking");
